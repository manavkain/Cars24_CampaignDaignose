...
export default function CampaignPulse({ compact = false }) {
  ...
  function applyForm(e) {
    e.preventDefault(); setMetrics({...metrics,...form}); setShowForm(false)
  }
  ...
  return (
    <div className="card" style={{ flexShrink: 0 }}>
      ...
      <div className="card-body" style={{display:'flex',flexDirection:'column',gap: compact ? 10 : 14, padding: compact ? '12px 16px' : '16px 20px' }}>
        {/* Stitch: Bento grid with background icons */}
          ...
          {METRICS.map(m=>{
            ...
            return (
              <div key={m.key} className="metric-card" style={{
                ...
                <div style={{position:'absolute',top:0,right:0,padding: compact ? 8 : 14, opacity:.04,pointerEvents:'none',transition:'opacity .3s'}}>
                ...
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom: compact ? 8 : 16}}>
                  ...
                  {d!==undefined && (
                    <span className={`badge ${hBadge[h]}`} style={{fontSize: 9, padding: '1px 5px'}}>
                      {Math.abs(d)}
                    </span>
                  )}
                </div>
                ...
                {!compact && <div style={{fontSize:11,color:'var(--outline)',marginTop:6,fontWeight:500}}>
        ...
        <div style={{display:'flex',gap:6}}>
          <input placeholder="Google Sheets URL..." value={sheetsUrl} onChange={e=>setSheetsUrl(e.target.value)} style={{fontSize: 11, padding: '7px 10px'}}/>
